...
# get cubist model ####
cubist <- readRDS("data/temp_results_cubist.rds") # from smax_learn/predict_temp_resamp_nogroup
cubist <- cubist[[1]] # the model
cubist$coefnames
...
weathlocal2 <- weathlocal %>% 
  ...
  mutate(
    ...
    SOL_CUM = cummean(SOLAR_MJH)
  ) %>% 
  ungroup() %>% 
  drop_na(AIR_C, AIR_MIN, HUM_PC, SOL_CUM, WIND_MPS) # needed for HSR
newdata <- weathlocal2 %>% 
  transmute(
    AIR_C = AIR_C, 
    AIR_MIN = AIR_MIN, 
...
weathlocal2$HSR <- predict(cubist, newdata = newdata) # slow!!!
weathlocal3 <- weathlocal2 %>% 
  group_by(STATION_ID, DATE) %>% 
  dplyr::filter(sum(!is.na(AIR_C + HUM_PC + SOLAR_MJH + SOL_CUM + WIND_MPS + 
                           THI + GHLI + GHLI2 + HSR)) == 24) %>%
  summarise(
    # daily max
    AIR_MIN = safemin(AIR_C),
    ...
    HSR_MAX = safemax(HSR),
    HSR_HOUR = safeget(HOUR, HSR == safemax(HSR)),
    # daily mean
    ...
    CUM_MAX = safeget(SOL_CUM, HOUR == AIR_HOUR),
    ...
    HSR_HRS = safesum(HSR > 25),
    HSR_HRS = ifelse(HSR_HRS == 24, NA, HSR_HRS),
    HSR_ACC = safesum(pmax(0, HSR - 25)),
  ) %>% 
  ungroup() %>% 
  mutate(
  ...
  drop_na(SOL_MAX) %>% 
  dplyr::filter(CUM_MAX/ SOL_MEAN > 0.05)
# hist(weathlocal3$SOL_MAX) # plausible
# hist(weathlocal3$CUM_MAX/weathlocal3$SOL_MEAN, breaks = 100) # plausible
# hist(weathlocal3$CUM_MAX) # plausible

# plot corr ####
if (FALSE){
  png("plots/weathmeanmaxcorr.png", width = 297, height = 210, units = "mm", res = 150)
  g <- ggpairs(weathlocal3, columns = which(str_detect(names(weathlocal3), "MAX|MEAN")))
  print(g)
  dev.off()
}

# modnames ####
modnames <- c("THI", "GHLI", "GHLI2", "HSR")
modnames <- c("THI", "GHLI", "HSR") # because honestly GHLI2 is not reliable yet
...
weathlocal3 %>% 
  ...
  mutate(
    ...
    name = factor(name, levels = modnames)
    ) %>% 
  drop_na(name) %>% 
  ggplot() +
  theme_bw() +
...
varnames <- c(
  ...
  SOL = "Solar Radiation (MJ/m2/h)", 
  CUM = "Cum. Mean Solar Radiation (MJ/m2/h)",
  WIND = "Wind Speed (m/s)"
)
  
# predict solar max ####
sol_mod <- lm(SOL_MAX ~ AIR_MAX + AIR_MIN + HUM_MEAN + WIND_MEAN + SOL_MEAN, data = weathlocal3)
summary(sol_mod)
sol_mod <- lm(SOL_MAX ~ 1 + SOL_MEAN, data = weathlocal3 %>% filter(SOL_MEAN>1))
summary(sol_mod)
sol_mod <- lm(SOL_MAX ~ 0 + SOL_MEAN, data = weathlocal3 %>% filter(SOL_MEAN>1))
summary(sol_mod)
weathlocal3$SOL_MAX2 <- predict(sol_mod, newdata = weathlocal3)
ggplot(weathlocal3) +
...
temp1 <- weathlocal3 %>% 
  ...
  mutate(var = varnames["SOL"])

# predict wind max ####
wind_mod <- lm(WIND_MAX ~ AIR_MAX + AIR_MIN + HUM_MEAN + WIND_MEAN + SOL_MEAN, data = weathlocal3)
summary(wind_mod)
wind_mod <- lm(WIND_MAX ~ 0 + WIND_MEAN, data = weathlocal3)
summary(wind_mod)
# wind_mod <- lm(WIND_MAX ~ 0 + WIND_MEAN + I(WIND_MEAN^2), data = weathlocal3)
# summary(wind_mod)
weathlocal3$WIND_MAX2 <- predict(wind_mod, newdata = weathlocal3)
ggplot(weathlocal3) +
...
temp2 <- weathlocal3 %>% 
  ...
  mutate(var = varnames["WIND"])

# predict humidity max ####
hum_mod <- lm(HUM_MAX ~ AIR_MAX + AIR_MIN + HUM_MEAN + WIND_MEAN + SOL_MEAN, data = weathlocal3)
summary(hum_mod)
hum_mod <- lm(I(100-HUM_MAX) ~ 0 + I(100-HUM_MEAN), data = weathlocal3)
summary(hum_mod)
weathlocal3$HUM_MAX2 <- 100-predict(hum_mod, newdata = weathlocal3)
ggplot(weathlocal3) +
...
temp3 <- weathlocal3 %>% 
  ...
  mutate(var = varnames["HUM"])

# predict cum sol ####
cum_mod <- lm(CUM_MAX ~ 0 + SOL_MEAN + I(SOL_MEAN^2), data = weathlocal3 %>% filter(SOL_MEAN>1))
summary(cum_mod)
weathlocal3$CUM_MAX2 <- predict(cum_mod, newdata = weathlocal3)
ggplot(weathlocal3) +
  geom_point(aes(x = SOL_MEAN, y = CUM_MAX)) +
  geom_abline(colour = "red") 
ggplot(weathlocal3) +
  labs(x = "Model for CUM_MAX") +
  geom_point(aes(x = CUM_MAX2, y = CUM_MAX)) +
  geom_abline(colour = "red") +
  coord_fixed()
myggsave("plots/weathcummaxmodel.png")
temp4 <- weathlocal3 %>% 
  dplyr::transmute(MEAN = SOL_MEAN, MAX = CUM_MAX, MODEL = CUM_MAX2) %>% 
  mutate(var = varnames["CUM"])
...
temp <- weathlocal3 %>% 
  ...
  mutate(
    ...
    HSR_MOD = predict(cubist, newdata = newdata)    
  ) %>%
  dplyr::select(AIR = AIR_MAX,
                THI_MAX, GHLI_MAX, GHLI2_MAX, HSR_MAX,
                THI_MOD, GHLI_MOD, GHLI2_MOD, HSR_MOD) %>% 
  ...
  mutate(
    Index = factor(Index, levels = modnames),
    Dummy = NULL,
    Model = unname(Model)
    ) %>% 
  drop_na(Index)
...
# compare thresholds #### 
temp %>% 
  dplyr::select(TMAX = AIR, name = Index, value = Data) %>% 
  mutate(
    name = factor(name, levels = modnames),
    crit = case_when(
      name == "THI" ~ 72,
  ...
  drop_na(name) %>% 
  dplyr::filter(TMAX >= 20) %>% 
  ggplot() +
  theme_bw() +
  labs(title = "", x = "Air Temperature (C)", y = "Heat Index Value", colour = "Index") +
  geom_hline(aes(yintercept = crit, colour = "Threshold")) +
  geom_point(aes(x = TMAX, y = value, colour = "Heat Index")) +
  geom_smooth(aes(x = TMAX, y = value, colour = "Regression"), method = "lm", se = FALSE) +
  scale_colour_manual(values = c("Heat Index" = "steelblue", "Threshold" = "black", "Regression" = "firebrick")) +
  scale_x_continuous(breaks = seq(20,30,5)) +
  facet_wrap(~ name, nrow = 1, scales = "free_y")
myggsave("plots/era_compare_indices2.png", height = 297/3)
...
temp <- weathlocal3 %>% 
  ...
  mutate(
    ...
    HSR_MOD = ifelse(HSR_MOD > 25, HSR_MOD, NA),
  ) %>%
  dplyr::select(THI_HRS, GHLI_HRS, GHLI2_HRS, HSR_HRS,
                THI_MOD, GHLI_MOD, GHLI2_MOD, HSR_MOD) %>% 
...
temp <- weathlocal3 %>% 
  ...
  dplyr::select(THI_ACC, GHLI_ACC, GHLI2_ACC, HSR_ACC,
                THI_MOD, GHLI_MOD, GHLI2_MOD, HSR_MOD) %>% 
